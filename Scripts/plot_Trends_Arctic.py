...
###############################################################################                  
        ...
        ### Mask data
        ensmeantrendm = ensmeantrend * lrpAAn
        ensmeantrendm[np.where(ensmeantrendm == 0)] = np.nan
...
###############################################################################     
        if variq == 'T2M':
            ...
            label = r'\textbf{[T2M-Trend : $^{\circ}$C]}'
        
        fig = plt.figure(figsize=(10,2))
        for r in range(len(modelGCMs)):
            var = ensmeantrend[r]
            varno = ensmeantrendm[r]
            ...
            cs1 = m.contourf(x,y,varno,limit,extend='both',antialiased=True)
        ...
        if AA == 'now':
            plt.savefig(directoryfigure + 'Arctic_Trends_AA.png',dpi=300)
        else:
            plt.savefig(directoryfigure + 'Arctic_Trends.png',dpi=300)
...
###############################################################################     
        if variq == 'T2M':
            ...
            barlim = np.round(np.arange(-2,3,1),2)
            cmap = cmocean.cm.balance
            label = r'\textbf{[T2M-TrendDiff : $^{\circ}$C]}'
        
        fig = plt.figure(figsize=(10,2))
        for r in range(len(modelGCMs)):
            var = ensmeantrend[r] - dectrendobs
            varno = ensmeantrendm[r] - dectrendobs
        ...
        if AA == 'now':
            plt.savefig(directoryfigure + 'Arctic_TrendsBias_AA.png',dpi=300)
        else:
            plt.savefig(directoryfigure + 'Arctic_TrendsBias.png',dpi=300)
...
###############################################################################     
        ...
        fig = plt.figure(figsize=(10,2))
        for r in range(len(modelGCMs)):
            var = ensmeantrend[-1] - ensmeantrend[r]
            varno = ensmeantrend[-1] - ensmeantrendm[r]
        ...
        if AA == 'now':
            plt.savefig(directoryfigure + 'Arctic_TrendsMMdiff_AA.png',dpi=300)
        else:
            plt.savefig(directoryfigure + 'Arctic_TrendsMMdiff.png',dpi=300)