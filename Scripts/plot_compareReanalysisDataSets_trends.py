"""
Script for creating TREND composites to compare the mean state of 
ERA5-BE and 20CRv3
...
def calcTrends(years,varq,lat,lon):
    trend = np.empty((lat.shape[0],lon.shape[0]))
    intercept = np.empty((lat.shape[0],lon.shape[0]))
    rvalue = np.empty((lat.shape[0],lon.shape[0]))
    pvalue = np.empty((lat.shape[0],lon.shape[0]))
    stderr = np.empty((lat.shape[0],lon.shape[0]))
    for i in range(lat.shape[0]):
        for j in range(lon.shape[0]):
            trend[i,j],intercept[i,j],rvalue[i,j],pvalue[i,j],stderr[i,j] = sts.linregress(years,varq[:,i,j])
...
hist_erat = calcTrends(yearsplotting[yearqhist],hist_era,lats_obs,lons_obs)
hist_20ct = calcTrends(yearsplotting[yearqhist],hist_20c,lats_obs,lons_obs)
hist_diff = hist_erat - hist_20ct
...
aa_erat = calcTrends(yearsplotting[yearqaa],aa_era,lats_obs,lons_obs)
aa_20ct = calcTrends(yearsplotting[yearqaa],aa_20c,lats_obs,lons_obs)
aa_diff = aa_erat - aa_20ct
    
### Read lat/lon grid
lon2,lat2 = np.meshgrid(lons_obs,lats_obs)

### Plotting arguments
difflimits = np.arange(-2,2.01,0.1)
diffbar = np.arange(-2,3,1)

complimits = np.arange(-2,2.01,0.1)
compbar = np.arange(-2,3,1)

trends = [hist_erat,hist_20ct,hist_diff,
              aa_erat,aa_20ct,aa_diff]
colormapsq = [cmocean.cm.balance,cmocean.cm.balance,cmocean.cm.balance,
...
for v in range(6):
    ...
    if v == 3:
        ...
        cbar.set_label(r'\textbf{T2M [$^{\circ}$C/decade]}',fontsize=9,color='k')   
        ...
        cbar = fig.colorbar(cs,cax=cbar_ax,orientation='horizontal',
                            extend='both',extendfrac=0.07,drawedges=False)    
        cbar.set_label(r'\textbf{T2M Difference [$^{\circ}$C/decade]}',fontsize=9,color='k')   
        cbar.set_ticks(barq[v])
        cbar.set_ticklabels(list(map(str,np.round(barq[v],2))))
        cbar.ax.tick_params(labelsize=6,pad=3) 
        ticklabs = cbar.ax.get_xticklabels()
        cbar.ax.set_xticklabels(ticklabs,ha='center')
        cbar.ax.tick_params(axis='x', size=.001)
...
plt.savefig(directoryfigure + 'ComparingReanalysis_trends.png',dpi=300)
