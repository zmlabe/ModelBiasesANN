...
for vv in range(1):
      for mo in range(1):
        ...
        rmse = np.load(directorydata + saveData + '_RMSE_standardizedData.npz')['arr_0'][:]
        ...
        rmseglo = np.load(directorydata + saveData + '_RMSEglo_standardizedData.npz')['arr_0'][:]
...
###############################################################################   
            ...
            plt.title(r'\textbf{RMSE(z) PER YEAR - %s}' % typemask,color='k',fontsize=15)
        
            plt.tight_layout()
            
            plt.savefig(directoryfigure + saveData + '_RawClimateModel_RMSE_standardizedData.png',dpi=300)
...
###############################################################################   
            ...
            plt.xlabel(r'\textbf{Average RMSE - [climate model data] - %s for %s for \underline{%s}}' % (monthlychoice,variq,region),color='dimgrey',fontsize=8,labelpad=8)
            plt.title(r'\textbf{RMSE(z-glo) PER YEAR - %s}' % typemask,color='k',fontsize=15)
        
            plt.tight_layout()
            
            plt.savefig(directoryfigure + saveData + '_RawClimateModel_RMSEglo_standardizedData.png',dpi=300)