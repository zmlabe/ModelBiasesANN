...
lrp = np.load(directorydata + 'LRPobs_ArcticALL_7classes_GLO_%s.npy' % dataset_obs,allow_pickle=True)
rawdata = np.load(directorydata + 'LRPobs_OBSCALED_7classes_GLO_%s.npy' % dataset_obs,allow_pickle=True)
obs_test = np.load(directorydata + 'Labels_LRPObs_%s_GLO.npy' % dataset_obs,allow_pickle=True)
...
if scaleLRPmax == True:
    plt.savefig(directoryfigure + 'MSFigure-5updated_v7_scaleLRP_ERA5BE_GLO.png',dpi=1000)
else:
    plt.savefig(directoryfigure + 'MSFigure-5updated_v7.png_ERA5BE_GLO',dpi=1000)