...
dataset_obs = 'ERA5BE'
allDataLabels = ['ERA5-BE','CanESM2','MPI','CSIRO-MK3.6','EC-EARTH','GFDL-CM3','GFDL-ESM2M','LENS']
...
###############################################################################     
        ...
        if variq == 'T2M':
            ...
            label = r'\textbf{std. dev. [$^{\circ}$C]}'
            
        fig = plt.figure(figsize=(8,1.5))
        for r in range(0,len(allDataLabels)):
            var = stdall[r]
            
            ax1 = plt.subplot(1,8,r+1)
        ...
        cbar1.ax.tick_params(axis='both', size=.001,labelsize=4.5,pad=2)
        cbar1.outline.set_edgecolor('dimgrey')
        
        plt.tight_layout()
        # plt.subplots_adjust(bottom=0.11,hspace=0.00)
        plt.savefig(directoryfigure + 'CLIMO_STDEV-%s_supplement.png' % saveData,dpi=900)
