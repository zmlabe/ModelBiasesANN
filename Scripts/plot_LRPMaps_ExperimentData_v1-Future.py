...
explainable AI for future data

Author     : Zachary M. Labe
Date       : 14 April 2021
Version    : 1 (Future data for RCP 8.5 from 2020 to 2099)
"""
...
for va in range(len(variablesall)):
    for m in range(len(pickSMILEall)):
        ...
        directoryfigure = '/Users/zlabe/Desktop/ModelComparison_v1/v2/Future/'
        ...
        if typeOfAnalysis == 'Experiment-4':
            plt.text(-1,0.00,r'\textbf{TRAINING ACCURACY = %s \%%}' % np.round(acctrain,1),color='k',
                  fontsize=7)
            plt.text(-1,-2,r'\textbf{TESTING ACCURACY = %s \%%}' % np.round(acctest,1),color='k',
                      fontsize=7)
        else:
        ...
        x=np.arange(2020,2099+1,1)
        plt.scatter(x,randpred,c=randpred,s=40,clip_on=False,cmap=cc.Antique_7.mpl_colormap,
                    edgecolor='k',linewidth=0.4,zorder=10)
        
        plt.xticks(np.arange(1950,2101,5),map(str,np.arange(1950,2101,5)),size=6)
        plt.yticks(np.arange(0,lenOfPicks+1,1),modelGCMsNames,size=6)
        plt.xlim([2020,2100])   
        plt.ylim([0,lenOfPicks-1])
