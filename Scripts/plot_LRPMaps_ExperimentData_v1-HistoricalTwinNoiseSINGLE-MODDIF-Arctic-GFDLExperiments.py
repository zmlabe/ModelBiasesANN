...
explainable AI for historical data for regional data AND uses GFDL-CM3 experi
...
for va in range(len(variablesall)):
    for m in range(len(pickSMILEall)):
        ...
        for xx in range(experinumber):
            ...
            label = r'\textbf{Relevance - [ %s ] - %s - %s}' % (variq,typeOfAnalysis,factorObs)
            ...
            cmap = cm.cubehelix2_16.mpl_colormap
            label = r'\textbf{Relevance - [ %s - OBS] - %s - %s}' % (variq,typeOfAnalysis,factorObs)
            ...
            plt.tight_layout()
            if AA == True:
                plt.savefig(directoryfigure + '%s/Regions/ObservationsData_%s_AA.png' % (typeOfAnalysis,saveData),dpi=300)
            else:
                plt.savefig(directoryfigure + '%s/Regions/ObservationsData_%s.png' % (typeOfAnalysis,saveData),dpi=300)
            ...
            plt.xlabel(r'\textbf{Predictions - [ %s - OBSERVATIONS ] - %s - %s}' % (variq,typeOfAnalysis,factorObs))
            ...
            plt.ylim([0,150])
            plt.xlabel(r'\textbf{Frequency [\%%] - [ %s - TESTING ] - %s - %s}' % (variq,typeOfAnalysis,factorObs))
            
            plt.tight_layout()
            if AA == True:
                plt.savefig(directoryfigure + '%s/Regions/PredictedModels_%s_AA.png' % (typeOfAnalysis,saveData),dpi=300)
            else:
                plt.savefig(directoryfigure + '%s/Regions/PredictedModels_%s.png' % (typeOfAnalysis,saveData),dpi=300)