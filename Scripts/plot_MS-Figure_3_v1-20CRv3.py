"""
Script to plot figure 3 with 20CRv3
...
### Read in data
globe = np.load(directorydata + 'Ranks_thresh-%s_%s_20C.npy' % (THRESH,'SMILEGlobe'))
arctic = np.load(directorydata + 'Ranks_thresh-%s_%s_20C.npy' % (THRESH,'LowerArctic'))

### Create empty years to match ERA5
empty = np.empty((len(allDataLabels),70-globe.shape[1]))
empty[:] = np.nan

globe = np.append(globe,empty,axis=1)
arctic = np.append(arctic,empty,axis=1)
...
for i in range(globe.shape[0]):
    for j in range(arctic.shape[1]-empty.shape[1]):
        cc = 'crimson'         
...
plt.savefig(directoryfigure + 'MS-Figure_3_v1-20CRv3.png',dpi=1000)
