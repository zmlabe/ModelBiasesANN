...
def readData(window,typemodel,variq,simuqq,land_only,reg_name,rm_standard_dev):
    ...
    data = Dataset(directorydata + 'LRP_Maps_ModelBiases-STDDEV%syrs_%s_Annual_%s_%s_land_only-%s_%s_STD-%s_%s.nc' % (window,typemodel,variq,simuqq,land_only,reg_name,rm_standard_dev,ensTypeExperi))
...
trainindices = np.int_(np.genfromtxt(directorydata + 'trainingEnsIndices_ModelBiases_%s_%s_%s_%s_iterations%s_STD-%s_%s.txt' % (variq,monthlychoice,reg_name,dataset,iterations[0],rm_standard_dev,ensTypeExperi)))
testindices = np.int_(np.genfromtxt(directorydata + 'testingEnsIndices_ModelBiases_%s_%s_%s_%s_iterations%s_STD-%s_%s.txt' % (variq,monthlychoice,reg_name,dataset,iterations[0],rm_standard_dev,ensTypeExperi)))
classel = np.int_(np.genfromtxt(directorydata + 'allClasses_ModelBiases_%s_%s_%s_%s-%s_iterations%s_STD-%s_%s.txt' % (variq,monthlychoice,reg_name,dataset_obs,dataset,iterations[0],rm_standard_dev,ensTypeExperi)))
...
plt.savefig(directoryfigure + 'Experiment2.png',dpi=300)