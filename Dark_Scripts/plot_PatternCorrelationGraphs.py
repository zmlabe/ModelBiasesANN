...
#       for mo in range(1):
        ...
        for ii in range(len(yrALLr)):
            ...
            plt.yticks(np.arange(0,12+1,1),modelGCMs,size=6,color='w')
            plt.xlim([1950,2020])   
            plt.ylim([0,option-1])
            
            plt.title(r'\textbf{PATTERN CORRELATIONS FOR [ %s ]}' % variables[vv],color='w',fontsize=15)
            ...
            for te in range(len(uni)):
                plt.text(2022,uni[te],r'\textbf{\#%s}' % cou[te],fontsize=7,color='w',
                          ha='center',va='center')
...
###############################################################################   
            ...
            plt.axhline(y=np.max(correnstrends),linestyle='--',linewidth=2,color='w',
            ...
            plt.xticks(np.arange(0,option+1,1),modelGCMs,size=4.5,color='w')
            plt.yticks(np.arange(-1,1.1,0.1),map(str,np.round(np.arange(-1,1.1,0.1),2)),size=6)
            plt.xlim([-0.5,option-1+0.5])   
            plt.ylim([-0.1,1])
            
            plt.xlabel(r'\textbf{highest R - [climate model data] - %s}' % (monthlychoice),color='darkgrey',fontsize=8,labelpad=8)
            plt.title(r'\textbf{...AND FOR LINEAR TRENDS - %s - %s}' % (region,typemask),color='w',fontsize=15)
        ...
        if option == 8:
            ...
            plt.xlabel(r'\textbf{Average R - [climate model data] - %s for %s}' % (monthlychoice,variq),color='darkgrey',fontsize=8,labelpad=8)
            plt.title(r'\textbf{PATTERN CORRELATIONS PER YEAR - %s - %s}' % (region,typemask),color='w',fontsize=15)
