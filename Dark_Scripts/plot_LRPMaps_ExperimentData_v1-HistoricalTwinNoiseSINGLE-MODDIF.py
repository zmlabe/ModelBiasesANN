...
for va in range(len(variablesall)):
    for m in range(len(pickSMILEall)):
        ...
        for r in range(lenOfPicks):
            ...
            if ocean_only == True:
                m.fillcontinents(color='dimgrey',lake_color='k')
            elif land_only == True:
                m.drawlsmask(land_color=(0,0,0,0),ocean_color='k',lakes=True,zorder=5)
        ...
        plt.text(-0.9,0.00,r'\textbf{TRAINING ACCURACY = %s \%%}' % np.round(acctrain,1),color='w',
              fontsize=7)
        plt.text(-0.9,-1,r'\textbf{TESTING ACCURACY = %s \%%}' % np.round(acctest,1),color='w',
                  fontsize=7)
        
        plt.savefig(directoryfigure + '%s_LRPComposites_%s.png' % (typeOfAnalysis,saveData),dpi=300)
        ...
        cmap = cm.cubehelix2_16.mpl_colormap
        label = r'\textbf{Relevance - [ %s - OBS]}' % (variq)
        ...
        ax1.annotate(r'\textbf{1950--2019}' ,xy=(0,0),xytext=(0.01,0.90),
                      textcoords='axes fraction',color='w',fontsize=11,
                      rotation=0,ha='center',va='center')        
        ...
        cbar1 = fig.colorbar(cs1,cax=cbar_ax1,orientation='horizontal',
                            extend='max',extendfrac=0.07,drawedges=False)
        cbar1.set_label(label,fontsize=5,color='darkgrey',labelpad=1.4)  
        cbar1.set_ticks(barlim)
        cbar1.set_ticklabels(list(map(str,barlim)))
        cbar1.ax.tick_params(axis='x', size=.01,labelsize=5)
        cbar1.outline.set_edgecolor('darkgrey')
        ...
        plt.tight_layout()
        plt.savefig(directoryfigure + '%s_ObservationsData_%s.png' % (typeOfAnalysis,saveData),dpi=300)
        ...
        plt.scatter(x,obspred,c=obspred,s=40,clip_on=False,cmap=cc.Antique_8.mpl_colormap,
                    edgecolor='w',linewidth=0.4,zorder=10)
        ...
        plt.xlabel(r'\textbf{Predictions - [ %s - OBSERVATIONS ]}' % (variq))
        ...
        plt.axhline(y=100,linestyle='--',linewidth=2,color='w',
        ...
        plt.ylim([0,150])
        plt.xlabel(r'\textbf{Frequency [\%%] - [ %s - TESTING ]}' % (variq))
        
        plt.tight_layout()
        plt.savefig(directoryfigure + '%s_PredictedModels_%s.png' % (typeOfAnalysis,saveData),dpi=300)