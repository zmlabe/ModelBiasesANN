...
plt.fill_between(x=yearsall,y1=mint,y2=maxt,facecolor='w',zorder=0,
         alpha=0.6,edgecolor='none',clip_on=False,label='MMLEA')
...
leg = plt.legend(shadow=False,fontsize=9,loc='upper center',
            bbox_to_anchor=(0.5, 0.08),fancybox=True,ncol=15,frameon=False,
            handlelength=1.5,handletextpad=0.25)
for line,text in zip(leg.get_lines(), leg.get_texts()):
    text.set_color(line.get_color())

plt.text(1996.3,-2.935,r'\textbf{MMLEA}',color='w',alpha=1,fontsize=9,zorder=20)    
plt.ylabel(r'\textbf{Near-Surface Temperature Anomaly [$^{\circ}$C]}',fontsize=8,color='w')
plt.title(r'\textbf{ARCTIC TEMPERATURE}',color='w',fontsize=28)
plt.yticks(np.arange(-4,4.5,0.5),map(str,np.round(np.arange(-4,4.5,0.5),2)),size=5)
plt.xticks(np.arange(1950,2021,10),map(str,np.arange(1950,2021,10)),size=5)
plt.xlim([1950,2020])   
plt.ylim([-3,3])
plt.tight_layout()
plt.savefig(directoryfigure + 'ArcticGMSTA_Comparison_LabeBarnes-2022ESS.png',dpi=600)