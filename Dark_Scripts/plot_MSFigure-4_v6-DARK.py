...
color = cm.cubehelix2_16.mpl_colormap(np.linspace(0.3,1,len(allDataLabels)))
...
plt.ylabel(r'\textbf{Confidence}',color='darkgrey',fontsize=7)
...
plt.plot(yearsall,glens,linewidth=1.5,color='w',alpha=1,zorder=3,clip_on=False,label=r'\textbf{LENS}',
...
plt.text(2024,50,r'\textbf{GLOBAL}',color='w',fontsize=20,rotation=270,
         ha='center',va='center')
plt.ylabel(r'\textbf{Frequency of Label}',color='darkgrey',fontsize=7)         
...
for line,text in zip(leg.get_lines(), leg.get_texts()):
    text.set_color(line.get_color())
plt.ylabel(r'\textbf{Frequency of Label}',color='darkgrey',fontsize=7) 
plt.text(2024,50,r'\textbf{ARCTIC}',color='w',fontsize=20,rotation=270,
         ha='center',va='center')          
        
plt.tight_layout()
plt.subplots_adjust(hspace=0.4)
plt.savefig(directoryfigure + 'MSFigure-4_DARK.png',dpi=1000)