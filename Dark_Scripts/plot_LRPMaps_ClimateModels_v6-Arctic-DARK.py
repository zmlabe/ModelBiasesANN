...
for va in range(len(variablesall)):
    for m in range(len(pickSMILEall)):
        ...
        # plt.tight_layout()
        # if AA == True:
        #     plt.savefig(directoryfigure + '%s/ObservationsData_%s_AA_DARK.png' % (typeOfAnalysis,saveData),dpi=300)
        # else:
        #     plt.savefig(directoryfigure + '%s/ObservationsData_%s_DARK.png' % (typeOfAnalysis,saveData),dpi=300)
        ...
        plt.scatter(x,obspred,c=obspred,s=40,clip_on=False,cmap=cm.cubehelix2_16_r.mpl_colormap,
                    edgecolor='darkgrey',linewidth=0.4,zorder=10)
        
        plt.xticks(np.arange(1950,2021,5),map(str,np.arange(1950,2021,5)),size=6,color='darkgrey')
        plt.yticks(np.arange(0,lenOfPicks+1,1),modelGCMsNames,size=6,color='w')
        plt.xlim([1950,2020])   
        ...
        ax.tick_params('y',length=4,width=2,which='major',color='darkgrey')
        ax.tick_params('x',length=0,width=0,which='major',color='darkgrey')
        ...
        plt.axhline(y=100,linestyle='--',linewidth=2,color='darkgrey',
                    clip_on=False,zorder=100,dashes=(1,0.3))
        
        ### Set color
        colorlist = [cm.cubehelix2_16_r.mpl_colormap(1/8),
                      cm.cubehelix2_16_r.mpl_colormap(0/8),
                      cm.cubehelix2_16_r.mpl_colormap(3/8),
                      cm.cubehelix2_16_r.mpl_colormap(5/8),
                      cm.cubehelix2_16_r.mpl_colormap(4/8),
                      cm.cubehelix2_16_r.mpl_colormap(6/8),
                      cm.cubehelix2_16_r.mpl_colormap(7/8),
                      cm.cubehelix2_16_r.mpl_colormap(8/8)]
        for i in range(lenOfPicks):
            rects[i].set_color(colorlist[i])
            rects[i].set_edgecolor(colorlist[i])
        
        plt.xticks(np.arange(0,lenOfPicks+1,1),modelGCMsNames,size=6,color='darkgrey')
        plt.yticks(np.arange(0,520,25),map(str,np.arange(0,520,25)),size=6,color='w')
        plt.xlim([-0.5,lenOfPicks-1+0.5])   
        plt.ylim([0,150])
        plt.xlabel(r'\textbf{OBSERVATIONS}')
        
        plt.tight_layout()
        if AA == True:
            plt.savefig(directoryfigure + 'PredictedModels_%s_AA_DARK.png' % (saveData),dpi=300)
        else:
            plt.savefig(directoryfigure + 'PredictedModels_%s_DARK.png' % (saveData),dpi=300)