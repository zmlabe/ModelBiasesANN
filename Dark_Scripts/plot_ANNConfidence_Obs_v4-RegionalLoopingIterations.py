...
color=cc.Vivid_6.mpl_colormap(np.linspace(0,1,len(regions)))
for r,c in zip(range(len(regions)),color):
    ...
    plt.text(1950,0.31,r'\textbf{%s}' % regionnames[r],color='w',
             fontsize=15,ha='left')  
    
    if r == 0:
        plt.text(1930,-0.1,r'\textbf{Maximum Confidence}',color='darkgrey',
                 fontsize=11,ha='left',rotation=90)       
        
# plt.tight_layout()
# plt.subplots_adjust(bottom=0.15)
plt.savefig(directoryfigure + '%s_RegionsMaxConfidence.png' % (typeOfAnalysis),dpi=300)